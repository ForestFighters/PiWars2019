...
use self::image::{imageops, DynamicImage};
    ...
    pub fn clear_colour(&mut self, colour: u8) {
        for i in 0..(self.lcd_width / 2 * self.lcd_height) {
            if i % 2 == 0 {
                ...
                self.poled_buf[i as usize] =
                    (colour & 0x0f) as u8 | self.poled_buf[i as usize] as u8;
            }
        }
        self.old_poled_buf = self.poled_buf.clone();
    }
    ...
    fn draw_image(&mut self, x1: i16, y1: i16, img: DynamicImage) -> Result<(), String> {
        ...
        for img_x in 0..width {
            for img_y in 0..height {
                ...
                //println!("{:#?} ",shade);
                self.draw_colour(x + img_x as i16, y + img_y as i16, shade)
                    .unwrap();
            }
        }
        return Ok(());
